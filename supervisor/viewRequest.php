                  ...
                  <div class="card " style="box-shadow: 0 0 .5rem rgba(0, 0, 0, .2); ">
                    <div class="card-body">
                      ...
                      <table class="table datatables table-bordered table-hover" id="dataTable-1">
                        ...
                        <?php
                                $no = 1;
                                $post = $con->prepare("select r.*, p.* , s.* from request r JOIN post p ON r.postID = p.postID JOIN student s ON s.regNo = r.studentID where s.instituteName = ? && statuss = ?  order by requestID  desc ");
                                $post->execute(array($capture->officeName, "Accepted"));
                        ...
                        ?>
                          <tr>
                            ...
                            <td>
                                  <a href="acceptedRequest.php?rid=<?php echo $row->requestID; ?>" class="btn btn-warning text-white mb-3" title="Assign supervisor"><span class="fe fe-check-square fe-16"></span></a>
                            </td>
                          </tr>
                        <?php $no +=1; } ?>
